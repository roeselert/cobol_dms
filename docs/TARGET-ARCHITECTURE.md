...
`60EXTRC0` log every chat-completions exchange in human-readable form to the
worker's stdout — the outgoing request (endpoint, model, headers with the
bearer token redacted, and the system + user messages including the full OCR
text) and the response as received (curl/HTTP outcome plus the raw response
body, logged on success and failure alike so provider error bodies are
visible) — for diagnosing extractions that return nothing),
